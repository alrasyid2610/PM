...
class WoBudgetController extends Controller
{
    ...
    {
        if (!userCan('wo-budget-close', 'can_update')) {
            return response()->json(['message' => 'Anda tidak memiliki akses untuk menyelesaikan Plan Budget WO'], 403);
        }
        ...
        $belumBeres = 0;
        foreach ($items as $item) {
            $statuses = DB::table('wo_budget_actuals')
                ->where('id_budget_item', $item->id_budget_item)
        ...
        $totalActual = 0;
        foreach ($items as $item) {
            $totalActual += DB::table('wo_budget_actuals')
                ->where('id_budget_item', $item->id_budget_item)
